...
//Current Bug State, responding to above
struct BugStateStruct bugSystemState;

//Current Arbotix State, responding to above
struct ArbotixStateStruct arbSystemState;
...
//	DEBUG_PRINT("Blackboard: %s\n", psLongMsgNames[msg->header.messageType]);
	...
	case TICK_1S:
		...
		bugSystemState.state = msg->tickPayload.bugState;
		gettimeofday(&setSystemState.atTime, NULL);
		break;
	case ARB_STATE:
		arbSystemState.state = msg->bytePayload.value;
		gettimeofday(&arbSystemState.atTime, NULL);
		break;
	case NEW_SETTING:
...
void BlackboardInit()
{
	...
	bugSystemState.state = BUG_STATE_UNKNOWN;
	gettimeofday(&bugSystemState.atTime, NULL);

	arbSystemState.state = ARB_STATE_UNKNOWN;
	gettimeofday(&arbSystemState.atTime, NULL);
