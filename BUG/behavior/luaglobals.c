...
char *bugStateNames[] = BUG_STATE_NAMES;
...
int InitLuaGlobals(lua_State *L)
{
	...
	SetGlobal(L, "bugstate", (float) BUG_STATE_UNKNOWN);
	lua_createtable(L, BUG_STATE_COUNT, 0);		//BUG STATE constants table
	table = lua_gettop(L);
	for (i=0; i< BUG_STATE_COUNT; i++)
	{
		lua_pushstring(L, bugStateNames[i]);
		lua_pushnumber(L, i);
		lua_settable(L, table);
	}
	lua_setglobal(L, "bug");
	...
	SetGlobal(L, "arbstate", (float) ARB_STATE_UNKNOWN);

	lua_createtable(L, ARB_STATE_COUNT, 0);		//ARBOTIX STATE constants table
	table = lua_gettop(L);
	for (i=0; i< ARB_STATE_COUNT; i++)
	{
		lua_pushstring(L, arbStateNames[i]);
		lua_pushnumber(L, i);
		lua_settable(L, table);
	}
	lua_setglobal(L, "arbotix");
...
int UpdateGlobalsFromMessage(lua_State *L, psMessage_t *msg)
{
	...
	case TICK_1S:
		...
		SetGlobal(L, "bugstate", msg->tickPayload.bugState);

		printf("S=%s, B=%s, A=%s\n",
				systemStateNames[setSystemState.state],
				bugStateNames[bugSystemState.state],
				arbStateNames[arbSystemState.state]);

		InvokeScript("tickhook");
		break;
	case ARB_STATE:
		SetGlobal(L, "arbstate", msg->bytePayload.value);
		InvokeScript("arbstatehook");
		break;
	case MOVE:
