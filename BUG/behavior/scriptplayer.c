...
//lua call-backs
static int ChangeBugState(lua_State *L);	//ChangeBugState(bug.standby)
...
int InitScriptingSystem()
{
	...
	//register call-backs
	lua_pushcfunction(luaState, ChangeBugState);	//set internal state
	lua_setglobal(luaState, "ChangeBugState");
...
int InvokeScript(char *scriptName)
{
	...
	{
//		DEBUG_PRINT("overmind: calling %s\n", scriptName);
...
//change BUGSTATE
int lastSentState;
static int ChangeBugState(lua_State *L)
{
	psMessage_t msg;
	int newState = lua_tointeger(L, 1);
	if (newState != bugSystemState.state &&
			newState != lastSentState)
	{
		psInitPublish(msg, BUG_STATE);
		msg.bytePayload.value = newState;
		NewBrokerMessage(&msg);
		LogInfo("lua: ChangeBugState(%s)", bugStateNames[msg.bytePayload.value]);
