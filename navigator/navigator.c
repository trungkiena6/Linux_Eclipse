...
//thread to forward move commands to motors
void *NavMotorThread(void *arg);
...
int NavigatorInit()
{
	...
	pthread_t thread;
	int s = pthread_create(&thread, NULL, NavMotorThread, NULL);
	if (s != 0)
	{
		LogError("MotorThread create failed. %i", s);
		return -1;
	}
...
void *NavMotorThread(void *arg)
{
	...
	LogRoutine("Motor thread ready");

	//loop
	while(1)
	{
		if  (fidoSystemState.state == FIDO_ACTIVE)
		{
			//ready to go
			//critical section
